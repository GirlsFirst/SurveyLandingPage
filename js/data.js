var classroom_names = [			{id: 0, text: 'Accenture Atlanta', speakers: [
				{id: 0, text: 'Atlanta Tech Village'},
				{id: 1, text: 'Clay Nelson'},
				{id: 2, text: 'Chae O\'Keefe'},
				{id: 3, text: 'Hadiyah-Nicole Green'},
				{id: 4, text: 'Other'}
		]},
		{id: 1, text: 'AT&T Atlanta: Ada Lovelace', speakers: [
				{id: 0, text: 'Georgia Power'},
				{id: 1, text: 'Titania Jordan'},
				{id: 2, text: 'Clay Nelson'},
				{id: 3, text: 'Josie Kressner'},
				{id: 4, text: 'Kelly Burton'},
				{id: 5, text: 'Other'}
		]},
		{id: 2, text: 'AT&T Atlanta: Grace Hopper', speakers: [
				{id: 0, text: 'Georgia Power'},
				{id: 1, text: 'Titania Jordan'},
				{id: 2, text: 'Korhonda Randolph'},
				{id: 3, text: 'Clay Nelson'},
				{id: 4, text: 'Other'}
		]},
		{id: 3, text: 'GE Atlanta', speakers: [
				{id: 0, text: 'Georgia Power'},
				{id: 1, text: 'Terri Chu'},
				{id: 2, text: 'Erin Martin'},
				{id: 3, text: 'Betsy Hare'},
				{id: 4, text: 'Other'}
		]},
		{id: 4, text: 'AT&T Austin', speakers: [
				{id: 0, text: 'Capital Factory'},
				{id: 1, text: 'UT Austin'},
				{id: 2, text: 'Patty Cifra'},
				{id: 3, text: 'Anita Johnson'},
				{id: 4, text: 'Lesley Clayton'},
				{id: 5, text: 'Other'}
		]},
		{id: 5, text: 'Barlovento Foundation @ Austin Community College', speakers: [
				{id: 0, text: 'Capital Factory'},
				{id: 1, text: 'Dell'},
				{id: 2, text: 'Ashley Lowry'},
				{id: 3, text: 'Anita Johnson'},
				{id: 4, text: 'Other'}
		]},
		{id: 6, text: 'The Barlovento Foundation @ Texas State University at Round Rock', speakers: [
				{id: 0, text: 'Capital Factory'},
				{id: 1, text: 'Dell'},
				{id: 2, text: 'Sina Bahram'},
				{id: 3, text: 'Adriana de la Cuadra'},
				{id: 4, text: 'Anita Salinas'},
				{id: 5, text: 'Other'}
		]},
		{id: 7, text: 'IBM Austin', speakers: [
				{id: 0, text: 'General Motors'},
				{id: 1, text: 'Capital Factory'},
				{id: 2, text: 'Patty Cifra'},
				{id: 3, text: 'Kavita Thakkar'},
				{id: 4, text: 'Kimberly Kelley'},
				{id: 5, text: 'Other'}
		]},
		{id: 8, text: 'Indeed Austin', speakers: [
				{id: 0, text: 'Capital Factory'},
				{id: 1, text: 'UT Austin'},
				{id: 2, text: 'Sina Bahram'},
				{id: 3, text: 'Ashley Lowry'},
				{id: 4, text: 'Lyndsey Seale'},
				{id: 5, text: 'Lesley Clayton'},
				{id: 6, text: 'Other'}
		]},
		{id: 9, text: 'Akamai Boston', speakers: [
				{id: 0, text: 'Fathom'},
				{id: 1, text: 'Kimberly Arcand'},
				{id: 2, text: 'Other'}
		]},
		{id: 10, text: 'Microsoft Boston', speakers: [
				{id: 0, text: 'Gabby Raymond'},
				{id: 1, text: 'Lauren Metter + Sarah Ribeiro'},
				{id: 2, text: 'Other'}
		]},
		{id: 11, text: 'TripAdvisor Boston', speakers: [
				{id: 0, text: 'Amazon Robotics'},
				{id: 1, text: 'Mary Marken'},
				{id: 2, text: 'Kimberly Arcand'},
				{id: 3, text: 'Other'}
		]},
		{id: 12, text: 'Twitter Boston', speakers: [
				{id: 0, text: 'Mary Marken'},
				{id: 1, text: 'Mojgan Lefevbre'},
				{id: 2, text: 'Mykim Dang'},
				{id: 3, text: 'Other'}
		]},
		{id: 13, text: 'Verizon Boston', speakers: [
				{id: 0, text: 'Lauren Metter + Sarah Ribeiro'},
				{id: 1, text: 'Mary Marken'},
				{id: 2, text: 'Elaine Call'},
				{id: 3, text: 'Other'}
		]},
		{id: 14, text: 'Accenture Chicago', speakers: [
				{id: 0, text: 'Groupon'},
				{id: 1, text: '1871'},
				{id: 2, text: 'Sheri Rubin'},
				{id: 3, text: 'Shalini Patel'},
				{id: 4, text: 'Other'}
		]},
		{id: 15, text: 'Girls Who Code @  the University of Illinois at Chicago: Ada Lovelace', speakers: [
				{id: 0, text: '1871'},
				{id: 1, text: 'Accenture'},
				{id: 2, text: 'Tasha Wilkins'},
				{id: 3, text: 'Star Cunningham'},
				{id: 4, text: 'Other'}
		]},
		{id: 16, text: 'Girls Who Code @ University of Illinois at Chicago: Grace Hopper', speakers: [
				{id: 0, text: 'Blue1647'},
				{id: 1, text: 'Accenture'},
				{id: 2, text: 'Tasha Wilkins'},
				{id: 3, text: 'Star Cunningham'},
				{id: 4, text: 'Other'}
		]},
		{id: 17, text: 'Microsoft Chicago', speakers: [
				{id: 0, text: 'Adler Planetarium'},
				{id: 1, text: 'Groupon'},
				{id: 2, text: 'Samantha Alaimo'},
				{id: 3, text: 'Sheri Rubin'},
				{id: 4, text: 'Ally Brisbin'},
				{id: 5, text: 'Other'}
		]},
		{id: 18, text: 'Synchrony Financial Chicago', speakers: [
				{id: 0, text: 'Adler Planetarium'},
				{id: 1, text: 'Groupon'},
				{id: 2, text: 'Praveen Yadav'},
				{id: 3, text: 'Sheri Rubin'},
				{id: 4, text: 'Jana Cohen Barbe'},
				{id: 5, text: 'Other'}
		]},
		{id: 19, text: 'Verizon Chicago', speakers: [
				{id: 0, text: 'Microsoft'},
				{id: 1, text: 'DePaul University Tour'},
				{id: 2, text: 'Groupon'},
				{id: 3, text: 'Alison Stanton'},
				{id: 4, text: 'Stella Garber'},
				{id: 5, text: 'Other'}
		]},
		{id: 20, text: 'AT&T Los Angeles', speakers: [
				{id: 0, text: 'Gibson Dunn'},
				{id: 1, text: 'Marie Smith'},
				{id: 2, text: 'Olivia Orrell-Jones'},
				{id: 3, text: 'Kimberly Scott'},
				{id: 4, text: 'Catherine George'},
				{id: 5, text: 'Other'}
		]},
		{id: 21, text: 'Disney Los Angeles', speakers: [
				{id: 0, text: 'SpaceX'},
				{id: 1, text: 'Kimberly Scott'},
				{id: 2, text: 'Liliana Monge'},
				{id: 3, text: 'Kayla Briet'},
				{id: 4, text: 'Other'}
		]},
		{id: 22, text: 'IBM Los Angeles', speakers: [
				{id: 0, text: 'Team One'},
				{id: 1, text: 'Liliana Monge'},
				{id: 2, text: 'Marie Smith'},
				{id: 3, text: 'Justin Washington'},
				{id: 4, text: 'Other'}
		]},
		{id: 23, text: 'Cheryl Saban Self-Worth Foundation for Women & Girls at USC Marshall School of Business: Ada Lovelace', speakers: [
				{id: 0, text: 'General Assembly'},
				{id: 1, text: 'Sony Pictures'},
				{id: 2, text: 'Lauren Mermel'},
				{id: 3, text: 'Lea Deesing'},
				{id: 4, text: 'Evette Vargas'},
				{id: 5, text: 'Kayla Briet'},
				{id: 6, text: 'Other'}
		]},
		{id: 24, text: 'Cheryl Saban Self-Worth Foundation for Women & Girls at USC Marshall School of Business: Grace Hopper', speakers: [
				{id: 0, text: 'General Assembly'},
				{id: 1, text: 'Sony Pictures'},
				{id: 2, text: 'Lauren Mermel'},
				{id: 3, text: 'Lea Deesing'},
				{id: 4, text: 'Evette Vargas'},
				{id: 5, text: 'Kayla Briet'},
				{id: 6, text: 'Other'}
		]},
		{id: 25, text: 'Knight Foundation at MDC Miami: Ada Lovelace', speakers: [
				{id: 0, text: 'Studio Center'},
				{id: 1, text: 'Niki Novo'},
				{id: 2, text: 'Pia Celestino'},
				{id: 3, text: 'Other'}
		]},
		{id: 26, text: 'Knight Foundation Miami @ FIU', speakers: [
				{id: 0, text: 'John Escobar'},
				{id: 1, text: 'Allison Cammack'},
				{id: 2, text: 'Pia Celestino'},
				{id: 3, text: 'Other'}
		]},
		{id: 27, text: ' Knight Foundation Miami: Grace Hopper', speakers: [
				{id: 0, text: 'Studio Center'},
				{id: 1, text: 'Allison Cammack'},
				{id: 2, text: 'Other'}
		]},
		{id: 28, text: 'Verizon Newark: Ada Lovelace', speakers: [
				{id: 0, text: 'NBCUniversal'},
				{id: 1, text: 'Marlyne Pierce'},
				{id: 2, text: 'Thomas Abend'},
				{id: 3, text: 'Other'}
		]},
		{id: 29, text: 'Verizon Newark: Grace Hopper', speakers: [
				{id: 0, text: 'NBCUniversal'},
				{id: 1, text: 'Jessica Sheft-Ason'},
				{id: 2, text: 'Ajaita Saini'},
				{id: 3, text: 'Other'}
		]},
		{id: 30, text: 'Accenture New York City', speakers: [
				{id: 0, text: 'New York Times'},
				{id: 1, text: 'Blippar'},
				{id: 2, text: 'Patricia Hernon'},
				{id: 3, text: 'Lorelle Reid'},
				{id: 4, text: 'Am_lie Lamont'},
				{id: 5, text: 'Other'}
		]},
		{id: 31, text: 'Adobe New York City', speakers: [
				{id: 0, text: 'New York Times'},
				{id: 1, text: 'Squarespace'},
				{id: 2, text: 'Lorelle Reid'},
				{id: 3, text: 'Kirti Agarwal'},
				{id: 4, text: 'Other'}
		]},
		{id: 32, text: 'AIG New York City: Ada Lovelace', speakers: [
				{id: 0, text: 'LinkedIn'},
				{id: 1, text: 'TechStars'},
				{id: 2, text: 'Samantha Quinones'},
				{id: 3, text: 'Hilary Winn'},
				{id: 4, text: 'Sabrina Varanelli'},
				{id: 5, text: 'Other'}
		]},
		{id: 33, text: 'AIG New York City: Grace Hopper', speakers: [
				{id: 0, text: 'LinkedIn'},
				{id: 1, text: 'TechStars'},
				{id: 2, text: 'Shradha Agarwal'},
				{id: 3, text: 'Other'}
		]},
		{id: 34, text: 'The AOL Charitable Foundation at the CUNY Advanced Science Research Center', speakers: [
				{id: 0, text: 'DataKind'},
				{id: 1, text: 'Adrienne Shulman'},
				{id: 2, text: 'Thomas Abend'},
				{id: 3, text: 'Other'}
		]},
		{id: 35, text: 'AppNexus New York City', speakers: [
				{id: 0, text: 'Ajaita Saini'},
				{id: 1, text: 'Samantha Quinones'},
				{id: 2, text: 'Other'}
		]},
		{id: 36, text: 'AT&T New York City', speakers: [
				{id: 0, text: 'Chanel'},
				{id: 1, text: 'Sunny Bates'},
				{id: 2, text: 'Adrienne Shulman'},
				{id: 3, text: 'Edith Mandel'},
				{id: 4, text: 'Other'}
		]},
		{id: 37, text: 'BlackRock New York City', speakers: [
				{id: 0, text: 'Squarespace'},
				{id: 1, text: 'Isabella Frassetti'},
				{id: 2, text: 'Christina Morillo'},
				{id: 3, text: 'Hilary Winn'},
				{id: 4, text: 'Other'}
		]},
		{id: 38, text: 'Goldman Sachs New York City: Ada Lovelace', speakers: [
				{id: 0, text: 'Refinery 29'},
				{id: 1, text: 'Caroline Sinders'},
				{id: 2, text: 'Amy Quispe'},
				{id: 3, text: 'Other'}
		]},
		{id: 39, text: 'Goldman Sachs New York City: Grace Hopper', speakers: [
				{id: 0, text: 'Refinery 29'},
				{id: 1, text: 'Caroline Sinders'},
				{id: 2, text: 'Amy Quispe'},
				{id: 3, text: 'Other'}
		]},
		{id: 40, text: 'IAC New York City', speakers: [
				{id: 0, text: 'Squarespace'},
				{id: 1, text: 'Amy Quispe'},
				{id: 2, text: 'Francesca Colombo'},
				{id: 3, text: 'Other'}
		]},
		{id: 41, text: 'IBM New York City', speakers: [
				{id: 0, text: 'News Corp'},
				{id: 1, text: 'Tiffany Xiao'},
				{id: 2, text: 'Am_lie Lamont'},
				{id: 3, text: 'Other'}
		]},
		{id: 42, text: 'JPMorgan Chase New York City', speakers: [
				{id: 0, text: 'Stevie Aligbe'},
				{id: 1, text: 'Tiffany Xiao'},
				{id: 2, text: 'Other'}
		]},
		{id: 43, text: 'Kate Spade & Company @ the Fashion Institute of Technology', speakers: [
				{id: 0, text: 'New York Times'},
				{id: 1, text: 'the Met'},
				{id: 2, text: 'Lindsay Hall'},
				{id: 3, text: 'Lorelle Reid'},
				{id: 4, text: 'Rebecca Garcia'},
				{id: 5, text: 'Other'}
		]},
		{id: 44, text: 'Moody\'s New York City', speakers: [
				{id: 0, text: 'Fueled Collective'},
				{id: 1, text: 'Christina Morillo'},
				{id: 2, text: 'Kim Goulbourne'},
				{id: 3, text: 'Other'}
		]},
		{id: 45, text: 'Microsoft New York City', speakers: [
				{id: 0, text: 'New York Times'},
				{id: 1, text: 'Katelyn Stanis'},
				{id: 2, text: 'Stevie Aligbe'},
				{id: 3, text: 'Other'}
		]},
		{id: 46, text: 'Viacom New York City', speakers: [
				{id: 0, text: 'News Corp'},
				{id: 1, text: 'Nathalie Leon'},
				{id: 2, text: 'Kelly Moran'},
				{id: 3, text: 'Rebecca Garcia'},
				{id: 4, text: 'Other'}
		]},
		{id: 47, text: 'Adobe Seattle', speakers: [
				{id: 0, text: 'Gabrielle Gutierrez'},
				{id: 1, text: 'Other'}
		]},
		{id: 48, text: 'Amazon Seattle', speakers: [
				{id: 0, text: 'Disney Interactive'},
				{id: 1, text: 'Gabrielle Gutierrez'},
				{id: 2, text: 'Mei Hsieh'},
				{id: 3, text: 'Tiffany Niver'},
				{id: 4, text: 'Other'}
		]},
		{id: 49, text: 'AT&T Seattle', speakers: [
				{id: 0, text: 'Seattle Times'},
				{id: 1, text: 'Gabrielle Gutierrez'},
				{id: 2, text: 'Laura Williams-Argilla'},
				{id: 3, text: 'Other'}
		]},
		{id: 50, text: 'Expedia Seattle', speakers: [
				{id: 0, text: 'Mel Carson'},
				{id: 1, text: 'Anna Marie Golden'},
				{id: 2, text: 'Other'}
		]},
		{id: 51, text: 'Groupon Seattle', speakers: [
				{id: 0, text: 'Pivotal Labs'},
				{id: 1, text: 'Seattle Times'},
				{id: 2, text: 'Laura Williams-Argilla'},
				{id: 3, text: 'Jane Odero Greene'},
				{id: 4, text: 'Other'}
		]},
		{id: 52, text: 'Microsoft Seattle', speakers: [
				{id: 0, text: 'Seattle Times'},
				{id: 1, text: 'Anna Marie Golden'},
				{id: 2, text: 'Jane Odero Greene'},
				{id: 3, text: 'Other'}
		]},
		{id: 53, text: 'Adobe San Jose: Ada Lovelace', speakers: [
				{id: 0, text: 'Yahoo!'},
				{id: 1, text: 'Diana Navarro'},
				{id: 2, text: 'Kirsten Wolberg'},
				{id: 3, text: 'Other'}
		]},
		{id: 54, text: 'Adobe San Jose: Grace Hopper', speakers: [
				{id: 0, text: 'Yahoo!'},
				{id: 1, text: 'Diana Navarro'},
				{id: 2, text: 'Kirsten Wolberg'},
				{id: 3, text: 'Other'}
		]},
		{id: 55, text: 'Adobe San Francisco', speakers: [
				{id: 0, text: 'Pinterest'},
				{id: 1, text: 'Alyssa Simpson'},
				{id: 2, text: 'Anna Kilgore'},
				{id: 3, text: 'Lucy Greco'},
				{id: 4, text: 'Albert Treat'},
				{id: 5, text: 'Other'}
		]},
		{id: 56, text: 'Amazon Bay Area', speakers: [
				{id: 0, text: 'Lifelock/Stanford'},
				{id: 1, text: 'Diana Navarro'},
				{id: 2, text: 'Paula Griffin'},
				{id: 3, text: 'Tejas Manohar'},
				{id: 4, text: 'Vindhya Sankapally'},
				{id: 5, text: 'Elynn Lee'},
				{id: 6, text: 'Other'}
		]},
		{id: 57, text: 'Autodesk San Francisco', speakers: [
				{id: 0, text: 'Pivotal Cloud Foundry'},
				{id: 1, text: 'Ford'},
				{id: 2, text: 'Aminatou Sow'},
				{id: 3, text: 'Kimber Lockhart'},
				{id: 4, text: 'Rod Begbie'},
				{id: 5, text: 'Natasha Vianna'},
				{id: 6, text: 'Other'}
		]},
		{id: 58, text: 'Electronic Arts Bay Area', speakers: [
				{id: 0, text: 'Ford'},
				{id: 1, text: 'Kimberly Unger'},
				{id: 2, text: 'Kaye Mason'},
				{id: 3, text: 'Kaye Mason'},
				{id: 4, text: 'Other'}
		]},
		{id: 59, text: 'Facebook Bay Area', speakers: [
				{id: 0, text: 'EA'},
				{id: 1, text: 'Robin Yang'},
				{id: 2, text: 'Andrea Ayres'},
				{id: 3, text: 'Yun Mou'},
				{id: 4, text: 'Paula Griffin'},
				{id: 5, text: 'Tejas Manohar'},
				{id: 6, text: 'Other'}
		]},
		{id: 60, text: 'GE Bay Area', speakers: [
				{id: 0, text: 'Blippar'},
				{id: 1, text: 'Other'}
		]},
		{id: 61, text: 'IBM San Jose', speakers: [
				{id: 0, text: 'Lifelock/Stanford'},
				{id: 1, text: 'Diana Navarro'},
				{id: 2, text: 'Cecile Cruz'},
				{id: 3, text: 'Cameron McClees'},
				{id: 4, text: 'Other'}
		]},
		{id: 62, text: 'Intuit Bay Area', speakers: [
				{id: 0, text: 'EA'},
				{id: 1, text: 'Kaye Mason'},
				{id: 2, text: 'Diana Navarro'},
				{id: 3, text: 'Other'}
		]},
		{id: 63, text: 'Moody\'s San Francisco', speakers: [
				{id: 0, text: 'Pinterest'},
				{id: 1, text: 'Kathleen Mullaney'},
				{id: 2, text: 'Brynne Herbert'},
				{id: 3, text: 'Julia Rhodes Davis'},
				{id: 4, text: 'Catherine George'},
				{id: 5, text: 'Other'}
		]},
		{id: 64, text: 'Microsoft Bay Area', speakers: [
				{id: 0, text: 'EA'},
				{id: 1, text: 'Lifelock/Stanford'},
				{id: 2, text: 'Robin Yang'},
				{id: 3, text: 'Andrea Ayres'},
				{id: 4, text: 'Sheryl Kline'},
				{id: 5, text: 'Other'}
		]},
		{id: 65, text: 'Pivotal Bay Area', speakers: [
				{id: 0, text: 'EA'},
				{id: 1, text: 'Lifelock/Stanford'},
				{id: 2, text: 'Autodesk'},
				{id: 3, text: 'Pivotal Cloud Foundry'},
				{id: 4, text: 'Kaye Mason'},
				{id: 5, text: 'Kimberly Unger'},
				{id: 6, text: 'Tejas Manohar'},
				{id: 7, text: 'Other'}
		]},
		{id: 66, text: 'Pixar Bay Area', speakers: [
				{id: 0, text: 'Dolby'},
				{id: 1, text: 'Sheryl Kline'},
				{id: 2, text: 'Lucia Greco'},
				{id: 3, text: 'Jeff Stern'},
				{id: 4, text: 'Other'}
		]},
		{id: 67, text: 'Prudential Newark', speakers: [
				{id: 0, text: 'Audible'},
				{id: 1, text: 'Marlyne Pierce'},
				{id: 2, text: 'Derica Curtis'},
				{id: 3, text: 'Cori Haines'},
				{id: 4, text: 'Solomon Steplight'},
				{id: 5, text: 'Other'}
		]},
		{id: 68, text: 'Sephora San Francisco', speakers: [
				{id: 0, text: 'Airbnb'},
				{id: 1, text: 'Ariana Mott'},
				{id: 2, text: 'Alyssa Simpson'},
				{id: 3, text: 'Allbert Treat'},
				{id: 4, text: 'Rod Begbie'},
				{id: 5, text: 'Other'}
		]},
		{id: 69, text: 'Square San Francisco', speakers: [
				{id: 0, text: 'Dolby'},
				{id: 1, text: 'Lifelock/Stanford'},
				{id: 2, text: 'Tasneem Misnadakis'},
				{id: 3, text: 'Claire Uhar'},
				{id: 4, text: 'Catherine Georges'},
				{id: 5, text: 'Other'}
		]},
		{id: 70, text: 'Twitter San Francisco', speakers: [
				{id: 0, text: 'Dolby'},
				{id: 1, text: 'Tasneem Misnadakis'},
				{id: 2, text: 'Claire Uhar'},
				{id: 3, text: 'Christine Matheney'},
				{id: 4, text: 'Other'}
		]},
		{id: 71, text: 'Workday Bay Area', speakers: [
				{id: 0, text: 'Radius'},
				{id: 1, text: 'Vindhya Sankapally'},
				{id: 2, text: 'Arlene Mordeno'},
				{id: 3, text: 'Christine Matheney'},
				{id: 4, text: 'Tejas Manohar'},
				{id: 5, text: 'Other'}
		]},
		{id: 72, text: 'AOL Washington DC', speakers: [
				{id: 0, text: '1776'},
				{id: 1, text: 'Lisa Hines'},
				{id: 2, text: 'Rhianon Anderson'},
				{id: 3, text: 'Other'}
		]},
		{id: 73, text: 'AT&T Washington DC', speakers: [
				{id: 0, text: 'Palantir'},
				{id: 1, text: 'Kelli Kedis Ogborn'},
				{id: 2, text: 'Rhianon Anderson'},
				{id: 3, text: 'Shannon Turner'},
				{id: 4, text: 'Other'}
		]},
		{id: 74, text: 'BSA Washington DC', speakers: [
				{id: 0, text: 'Reshma/Megan Smith Town Hall'},
				{id: 1, text: 'Kelli Kedis Ogborn'},
				{id: 2, text: 'Shannon Turner'},
				{id: 3, text: 'Lisa O\'Brien'},
				{id: 4, text: 'Other'}
		]},
		{id: 75, text: 'Capital One Washington DC', speakers: [
				{id: 0, text: '1776'},
				{id: 1, text: 'Reshma/Megan Smith Town Hall'},
				{id: 2, text: 'Lisa Hines'},
				{id: 3, text: 'Rhianon Anderson'},
				{id: 4, text: 'Linda Mills'},
				{id: 5, text: 'Other'}
		]},
		{id: 76, text: 'Lockheed Martin Washington DC: Ada Lovelace', speakers: [
				{id: 0, text: 'General Assembly'},
				{id: 1, text: 'Reshma/Megan Smith Town Hall'},
				{id: 2, text: 'Kelli Kedis Ogborn'},
				{id: 3, text: 'Other'}
		]},
		{id: 77, text: 'Lockheed Martin Washington DC: Grace Hopper', speakers: [
				{id: 0, text: 'General Assembly'},
				{id: 1, text: 'Reshma/Megan Smith Town Hall'},
				{id: 2, text: 'Shannon Turner'},
				{id: 3, text: 'Other'}
		]},
  { id: 78, text: 'Select a Classroom'}
	//We added a comment!
]

var surveysData = [ //Codes are for form assembly forms in order classroom, first name, last name, email
    { id: 0, text: "Pre-Program Survey", url: "https://www.tfaforms.com/424585", codes: ["tfa_297", "tfa_695", "tfa_988"]},
    { id: 1, text: 'Pre Evaluation', url: 'https://www.tfaforms.com/424137', codes: ["tfa_122", "tfa_124", "tfa_126", "tfa_128"]},
    { id: 2, text: 'Week 1 Check-in', url: "https://www.tfaforms.com/424576", codes: ["tfa_297", "tfa_695", "tfa_989", "tfa_991"]},
    { id: 3, text: 'Week 2 Check-in', url: "https://www.tfaforms.com/427845", codes: ["tfa_297", "tfa_695", "tfa_989", "tfa_991"]},
    { id: 4, text: 'Week 3 Check-in', url: "https://www.tfaforms.com/424572", codes: ["tfa_297", "tfa_695", "tfa_1028", "tfa_1030"]},
    { id: 5, text: "Mid-Program Survey", url: "https://www.tfaforms.com/424584", codes: ["tfa_297", "tfa_695", "tfa_988"]},
    { id: 6, text: 'Week 4 Check-in', url: "https://www.tfaforms.com/424574", codes: ["tfa_297", "tfa_695", "tfa_697",  "tfa_699"]},
    { id: 7, text: 'Post Evaluation', url: "https://www.tfaforms.com/424578", codes: ["tfa_256", "tfa_258", "tfa_260", "tfa_262"]},
    { id: 8, text: 'Post-Program Survey', url: "https://www.tfaforms.com/424586", codes: ["tfa_297", "tfa_695", "tfa_1028"]},
    { id: 9, text: 'Speaker & Field Trip Feedback', url: 'pe.html', codes: ["class_id", "first_name", "last_name"]},
		{ id: -1, text: "Select a Survey", url: "0", codes: ["0", "0", "0"]}
]

var teacherSurveysData = [ //Codes are for form assembly forms for teachers in order classroom, first name, last name
		{ id: 0, text: 'Speaker & Field Trip Feedback', url: 'teacherpe.html', codes: ["class_id", "first_name", "last_name", "email"]},
		{ id: 1, text: 'Day 1 Pulse Check', url: "https://docs.google.com/a/girlswhocode.com/forms/d/17PIaQE6h8OM5T7neydWYplziQVHYpA2cCKx8UvlkgUw/viewform", codes: ["entry.680875110", "entry.1320068170"]},
		{ id: 2, text: "Week 1 Feedback", url: "https://docs.google.com/a/girlswhocode.com/forms/d/1kzOoOtdrkA8HwhQtdWEQmCHr3XgxrqMwzmM3NqORDz4/viewform", codes: ["entry.383924707", "entry.333065206"]},
		{ id: 3, text: "Week 2 Feedback", url: "https://docs.google.com/a/girlswhocode.com/forms/d/1ByRB-QAXJEaGtOosG7kCwwFOkxNe9_lwmBtgfsHjCHQ/viewform", codes: ["entry.383924707", "entry.333065206"]},
		{ id: 4, text: "Week 3 Feedback", url: "https://docs.google.com/a/girlswhocode.com/forms/d/1k5JfSmfCEnFGFq2ft8OIatfRyhndFoWBvJX9vhPbiHM/viewform", codes: ["entry.383924707", "entry.333065206"]},
		{ id: 5, text: "Week 4 Feedback", url: "https://docs.google.com/a/girlswhocode.com/forms/d/1q9YHiqOR5tqLK4rMyFpNMgDuNXNyS00y57zWt92tME4/viewform", codes: ["entry.383924707", "entry.333065206"]},
		{ id: 6, text: "Week 5 Feedback", url: "https://docs.google.com/a/girlswhocode.com/forms/d/1mUZDj_mY-CdDAYw5KaXCSfuUShqCgLVWgT-8QtO01aI/viewform", codes: ["entry.383924707", "entry.333065206"]},

		{ id: -1, text: "Select a Survey", url: "0", codes: ["0", "0", "0"]}
]
